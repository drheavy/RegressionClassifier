...
class TestEnsemble:
    def test_fit_two_bins_two_levels(self):
        ...
        model = ClassRegressorEnsemble(n_bins=2, n_levels=2, bins_calc_method='percentile')
    ...
    def test_fit_many_levels_better_than_dummy(self, airbnb_split):
        X_train_scaled, X_test_scaled, y_train, y_test = airbnb_split
        model = ClassRegressorEnsemble(n_bins=2, n_levels=4, bins_calc_method='percentile')
        model.fit(X_train_scaled, y_train)
